...
class HomeController extends Controller
{
    ...
    public function history() {
        ...
        $all_order = DB::table('tbl_order')
            ...
            ->where('tbl_order.customer_id', '=', $user_id)
            ->select('tbl_order.*', 'tbl_customers.customer_name')
            ->orderBy('tbl_order.order_id', 'desc')
            ->get();
    
        // Tiếp theo, trả về view với dữ liệu đã lấy được
        return view('product.history', compact('all_order'));
    }
    ...
    public function nhabanle(){
        $all_order = DB::table('tbl_order')
        ->join('tbl_customers', 'tbl_order.customer_id', '=','tbl_customers.customer_id')   
        ->select('tbl_order.*','tbl_customers.customer_name')
        ->orderby('tbl_order.order_id', 'desc')->get();
        $manager_order = view('product.nhabanle')->with('all_order',$all_order);
        return view('product.nhabanle', data:compact(var_name: 'all_order')); 
    }
    public function view_truyxuat($orderId){
        $order_by_id = DB::table('tbl_order')
        ->join('tbl_customers', 'tbl_order.customer_id', '=','tbl_customers.customer_id')
        ->join('tbl_shipping', 'tbl_order.shipping_id', '=','tbl_shipping.shipping_id')
        ->join('tbl_order_details', 'tbl_order.order_id', '=','tbl_order_details.order_id')
        ...
        $manager_order_by_id = view('product.truyxuat')->with('order_by_id',$order_by_id);
        return view('product.truyxuat', data:compact(var_name: 'order_by_id'));
    }
